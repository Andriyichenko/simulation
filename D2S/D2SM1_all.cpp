// D2SM1 with x_0=1, T=1, a=0.5, b=0.5
...
int main() {  
    ...
    const string csv_path = dir_path + "/D2SM1_all_100_1000_data.csv"; //data sourceのファイル名指定
    ...
    for (int n = 0; n <= max_n; ++n) {
       ...
       #pragma omp parallel reduction(+:S, Sm, S_1_5, Sb, B, Bm, B_1_5, Bb)
        {
            // 各threadは独自の乱数生成器を持つ
            mt19937 rng(42),rng1(30), rng2(50);
            ...
            for (int p = 0; p < paths; ++p) {
                ...
                for (int idx = 1; idx < points; ++idx) {
                    // ランダム数の生成
                    const double Z = dist(rng), Z1 = dist(rng1), Z2 = dist(rng2);
                    const double Z2_sqrt_dt = sqrt_dt * Z2, Z1_sqrt_dt = sqrt_dt * Z1;
