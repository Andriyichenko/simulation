// D2fM1_check with x_0=1, T=1, a=0.5, b=0.5
...
int main() {  
    ...
    const string csv_path = dir_path + "/D2fM1_check_100_1000_data.csv"; //data sourceのファイル名指定
    ...
    for (int n = 0; n <= max_n; ++n) {
        ...
        // {
            ...
            dX0  = f(sum_W) / sqrt(dt);    
            dX1 = f(sum_W1) / sqrt(dt);   
            dX2 = f(sum_W2) / sqrt(dt); 
            dX_b = f(sum_Xb) / sqrt(dt);    
