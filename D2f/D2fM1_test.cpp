...
inline double cal_sp_W_stateb(const StateCoeff& coef) {
    return 1.0 / coef.sigma_sq;
}
...
int main() {  
    ...
    const string csv_path = dir_path + "/D2fM1_100_1000_data_test.csv"; //data sourceのファイル名指定
    ...
    for (int n = 0; n <= max_n; ++n) {
        const int points = 1 + 1 * n; //(10-50-100-200-400-600-800-1000)
    ...
    //     {
            ...
            for (int p = 0; p < paths; ++p) {
                ...
                for (int idx = 1; idx < points; ++idx) {
                    ...
                    double sp_W_stateb = cal_sp_W_stateb(coef_X_b);
