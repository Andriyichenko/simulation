...
inline double benchmark_opt(double X_b, double dt, double Z, double b) {
    const double asinh_Xb = asinh(X_b);
    const double sqrt_dt = sqrt(dt);
   

    return sinh((asinh_Xb) + 0.5 * dt + sqrt_dt * Z);

}

inline double compute_sum_state(double delta_val) {
    return delta_val * (1.0 - 0.5 * delta_val);  // 优化：x - 0.5*x^2 = x(1-0.5x)
}
...
int main() {  
    ...
    const string csv_path = dir_path + "/LTM1_10_100_data.csv"; //data sourceのファイル名指定
        ...
        #pragma omp parallel reduction(+:S,Sm,S_1_5,Sb,B,Bm,B_1_5,Bb) //OpenMPのreduction句で指定の変数の和(+:)を並列計算 i.e. reduction(operator : variable_list)
        {
            ...
            for (int p = 0; p < paths; ++p) {
                ...
                for (int idx = 1; idx < points; ++idx) {
                    ...
                    coef_m.compute( a, b, W_state, 0.0);
                    coef_1_5.compute( a, b, W_state, 0.0);
                    coef_X_b.compute( a, b, W_state, 0.0);
                    ...
                    X_b_Y = benchmark_opt(X_b, dt,Z, b);
