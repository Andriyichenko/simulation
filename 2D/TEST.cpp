...
static inline double var_nonneg_from_sums(long double sum, long double sumsq, long double N) {
    const long double mean  = sum   / N;   // E[X]
    const long double mean2 = sumsq / N;   // E[X^2]
    long double var = std::fma(-mean, mean, mean2); // mean2 - mean*mean (更稳)
    if (var < 0) var = 0;                  // 保证非负（截断浮点伪负数）
...
// a(x) = [a1(x), a2(x)]^T = [x1, x2]^T
// ========================================
inline State a_func(const State& x) {
    return State(1.0, 1.0); // return State(x(0), x(1));
}

// Partial derivatives: ∂i aj(x)
inline double d_a1_dx1(const State&) { return 0.0; }
inline double d_a1_dx2(const State&) { return 0.0; }
inline double d_a2_dx1(const State&) { return 0.0; }
...
inline double Sigma1(const State& x) {
    const double x1 = 1.0; // x(0);
    const double x2 = 1.0; // x(1);
    ...
    const double a1 = a(0); //const double a1 = a(0);
    const double a2 = a(1);
...
inline double Delta2(const State& x, const State& y, double t) {
    ...
    double da1_dx1 = d_a1_dx1(x);
    double da1_dx2 = d_a1_dx2(x);
    double da2_dx1 = d_a2_dx1(x);
    double da2_dx2 = d_a2_dx2(x);
    ...
    double L7 = (t3 / 12.0) * (
        ds_x1 * ds_x2 * s_x1 * pow(s_x2, 3) * h1112 +
        ds_x1 * ds_x2 * pow(s_x1, 3) * s_x2 * h1222
    );
...
int main() {  
    ...
    for (int n = 0; n <= max_n; ++n) {
        ...
        A[n]     = (double)(S     / N);
        Am[n]    = (double)(Sm    / N);
        A_1_5[n] = (double)(S_1_5 / N);
        A_lim[n] = (double)(S_lim / N);
