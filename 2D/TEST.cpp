...
inline State A0(const State& curr, double dt, double Z1, double Z2) {
    ...
    const State drift = a_func(curr);

    // Model 3 diffusion structure: [ s(x2) 0; 0 s(x1) ] dW
...
inline State A2(const State& curr, double dt, double Z1, double Z2) {
    Vector2d dW(sqrt(dt) * Z1, sqrt(dt) * Z2);
...
int main() {
    ...
    const string csv_path = dir_path + "/TEST_2DD2SM3_all_s2sin_100_1000_data.csv";
    ...
    for (int n = 0; n <= max_n; ++n) {
        ...
        #pragma omp parallel reduction(+:S,Sm,S_1_5,S_lim,B,Bm,B_1_5,B_lim)
        {
            ...
            for (int p = 0; p < paths; ++p) {
                ...
                // calculate the Sigma^1 term integral
                ...
                for (int idx = 1; idx < points; ++idx) {
                    ...
                    for (int m = 0; m < points; ++m) {
                        ...
                        // Sigma^1(x) from PDF p.71, expressed via a(x), s(x)
                        double Sigma_val = Sigma1(st_nm);
                        Integrated_Sigma += Sigma_val * dtm;

                        // Update Benchmark State (Milstein at fine scale)
                        State nm_benchmark = A1(st_nm, dtm, Z1_nm, Z2_nm);
                        st_nm = nm_benchmark;

                        // Aggregate Brownian increments for coarse step
