    ...
    for (int n = 0; n <= max_n; ++n) {
        ...
        #pragma omp parallel reduction(+:S,Sm,S_1_5,B,Bm,B_1_5) 
        {
            ...
            for (int p = 0; p < paths; ++p) {
                ...
                //slide benchmark fomula of D4
                for (int m = 0; m < nm_points; ++m){
                    mt19937 rng_nm(3 + p + m); 
                    mt19937 rng1_nm(7 + p + m);
                    double Z1_nm = dist(rng_nm);
                    double Z2_nm = dist(rng1_nm);
                    State nm_benchmark = A1(st_nm, dtm, Z1_nm, Z2_nm);
                    L_nm += phi_n(z_const, alpha, nm_benchmark(0), dtm);
                    st_nm = nm_benchmark;
                }
