...
//   sigma ≈ 0.596
//
// Approximation (eq. after 4.20):
//   L_t^{mu_n,n} = 1/(n*Leb(F^(k_n))) * sum_{k=1}^{floor(nt)} 1_{F^(k_n)}(X_{k/n})
//
// Code format follows 2DLT_koch.cpp exactly.
...
// 点 x が三角形 [v0,v1,v2] の内部(辺上含む)にあるか (バリセントリック座標法)
...
// 1_{F^(k)}(x): x が F^(k) のいずれかの三角形内部/辺上にあるか
...
// ================================================================
// Leb(F^(k)) = (sqrt(3)/4) * (3/4)^k   [論文 eq. after (4.21)]
// ================================================================
...
int main() {
    ...
    const string csv_path = dir_path + "/2DLTM3_gasket_error_100_1000_data.csv";
