...
int main() {  
    ...
    for (int n = 0; n <= max_n; ++n) {
        ...
        // パラメータ初期化
        long double S = 0.0L, Sm = 0.0L, S_1_5 = 0.0L, S_nv = 0.0L;
        long double B = 0.0L, Bm = 0.0L, B_1_5 = 0.0L, B_nv = 0.0L;

        // OpenMP threadの並列化
        #pragma omp parallel reduction(+:S,Sm,S_1_5,S_nv,B,Bm,B_1_5,B_nv) 
        {
            normal_distribution<double> dist_nm0(mu, sigma);
            ...
            for (int p = 0; p < paths; ++p) {
                ...
                seed_seq ss2{54u, 2u, (uint32_t)p}; // Seed for xi generator
                ...
                for (int idx = 1; idx < points; ++idx) {
                    ...
                    L_nv  += dt * phi_n(z_const, alpha, next_nv(0), dt);
