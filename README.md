...
##### Ninomiya–Victoir Scheme for Model 3

For Model 3, the stochastic dynamics are built from three vector fields,

$$
V_1(x) = \begin{pmatrix} s(x_2) \\ 0 \end{pmatrix}, \qquad
V_2(x) = \begin{pmatrix} 0 \\ s(x_1) \end{pmatrix}, \qquad
V_0(x) = a(x) = \begin{pmatrix} x_2 \\ -x_1 \end{pmatrix}.
$$

The diffusion matrix is diagonal and the two diffusion vector fields act in orthogonal directions. Because $V_1$ depends only on $x_2$ and points in the $x_1$-direction, while $V_2$ depends only on $x_1$ and points in the $x_2$-direction, the Stratonovich correction vanishes here:

$$
(\partial V_1)V_1 = 0, \qquad (\partial V_2)V_2 = 0.
$$

So, for this model, the Itô and Stratonovich drifts coincide, and we may use the same drift field $V_0=a$ in the Ninomiya–Victoir splitting.

The NV scheme is based on the exact flows of these vector fields. The diffusion flows are particularly simple:

$$
\exp(zV_1)(x_1,x_2) = (x_1 + z\,s(x_2),\ x_2),
$$

$$
\exp(zV_2)(x_1,x_2) = (x_1,\ x_2 + z\,s(x_1)).
$$

The drift flow is the rotation flow generated by $V_0$:

$$
\exp(tV_0)(x_1,x_2)
= (x_1\cos t + x_2\sin t,\ -x_1\sin t + x_2\cos t).
$$

Equivalently, if we define the half-drift flow and the two diffusion flows by

$$
\varphi_0^{h/2}(x) = \exp\!\left(\frac{h}{2}V_0\right)x, \qquad
\varphi_1^z(x) = \exp(zV_1)x, \qquad
\varphi_2^z(x) = \exp(zV_2)x,
$$
...
where $\eta_k \in \{-1,+1\}$ is a Rademacher random variable independent of the Brownian increments. This randomized ordering is the key feature of the Ninomiya–Victoir scheme and is what makes the method particularly effective for weak approximation.

In this project, the 2D files with the `NVM3` substring use this NV construction together with the three approximation levels $\bar{X}^0$, $\bar{X}^1$, and $\bar{X}^2$. In other words, they are the Model 3 comparison programs that place the classical schemes and the NV scheme side by side under the same test functional.
